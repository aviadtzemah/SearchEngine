...
class MultiFileWriter:
    ...
    def upload_to_gcp(self):
        ...
        blob = self.bucket.blob(f"title_postings_gcp/{file_name}")
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                self._open_files[f_name] = open(f"D:/University/InformationRetrieval/Project/data_to_work_with/anchor_postings_gcp/{f_name}", 'rb')
...
class InvertedIndex:
    ...
    def posting_lists_iter(self, base_dir):
        ...
        with closing(MultiFileReader()) as reader:
            for w, locs in self.posting_locs.items():
                b = reader.read(base_dir, locs, self.df[w] * TUPLE_SIZE)
                posting_list = []
    ...
    def posting_lists_tokes(self, tokens):
        with closing(MultiFileReader()) as reader:
            postings = []
            exists = []
            for token in tokens:
    ...
    def write_a_posting_list(b_w_pl, bucket_name):
        ...
        with closing(MultiFileWriter(".", bucket_id, bucket_name)) as writer:
            for w, pl in list_w_pl:
                # convert to bytes
                b = b''.join([(doc_id).to_bytes(TUPLE_SIZE, 'big')
                              for doc_id in pl])
                # write to file(s)
                locs = writer.write(b)
    ...
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name):
        ...
        # blob_posting_locs = bucket.blob(f"title_postings_gcp/{bucket_id}_posting_locs.pickle")
...
#         '''
#         file_name = self._f.name
#         blob = self.bucket.blob(f"anchor_postings_gcp/{file_name}")
#         blob.upload_from_filename(file_name)
#
#
#
...
#         client = storage.Client()
#         bucket = client.bucket(bucket_name)
#         blob_posting_locs = bucket.blob(f"anchor_postings_gcp/{bucket_id}_posting_locs.pickle")
#         blob_posting_locs.upload_from_filename(f"{bucket_id}_posting_locs.pickle")
