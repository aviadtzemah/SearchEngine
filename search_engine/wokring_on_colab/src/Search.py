...
def weighted_search_df(query, body_weight = 0.25, title_weight = 0.25, anchor_weight = 0.2, pagerank_weight = 0.25, pageviews_weight = 0.25, N=100):
  """ 
  ...
  scaler = MinMaxScaler() # assuming that using the same scalar won't make any problems

  title_results = search_title_df(query) 
  title_results['score'] = scaler.fit_transform(title_results[['score']]) * title_weight  # normalizing the results and multiplying by the weight

#   anchor_results = search_anchor_df(query) TODO: currently without anchor
#   anchor_results['score'] = scaler.fit_transform(anchor_results[['score']]) * anchor_weight # normalizing the results and multiplying by the weight

  search_result = body_results.add(title_results, fill_value=0) #.add(anchor_results, fill_value=0) TODO: currently without anchor

  article_ids = search_result.index.tolist()
  # adding pagenk score
  pageranks = get_pagerank_df(article_ids).rename(columns={'rank': 'score'}) # TODO: maybe just change the name of the col in its function?
  ...
  pageviews = get_pageview_df(article_ids).rename(columns={'views': 'score'}) # TODO: maybe just change the name of the col in its function?
  pageviews['score'] = scaler.fit_transform(pageviews[['score']]) * pageviews_weight
  search_result = search_result.add(pageviews)

  return search_result.nlargest(N, 'score')[['score']].to_records(index=True)
...
def search_wiki(query):
  ...
  '''
  return weighted_search_df(query)