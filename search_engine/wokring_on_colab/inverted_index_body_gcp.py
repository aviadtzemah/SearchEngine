...
class MultiFileWriter:
    ...
    def upload_to_gcp(self):
        ...
        blob = self.bucket.blob(f"correct_body_postings_gcp/{file_name}")
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                self._open_files[f_name] = open(
                    f"D:/University/Information Retrieval/Project/data_to_work_with/correct_body_postings_gcp/{f_name}", 'rb')
...
# many bytes.
TF_MASK = 2 ** 32 - 1  # Masking the 32 (4 bytes) low bits of a float
...
class InvertedIndex:
        ...
        with closing(MultiFileReader()) as reader:
            ...
            for token in tokens:
                if token in self.posting_locs:
                    ...
                    b = reader.read(locs[0], self.df[token] * TUPLE_SIZE)
                    print(len(b))
                    posting_list = []
    ...
    def write_a_posting_list(b_w_pl, bucket_name):
        ...
        with closing(MultiFileWriter(".", bucket_id, bucket_name)) as writer:
            for w, pl in list_w_pl:
                ...
                #               for doc_id, tf in pl])
                b = b''.join([(doc_id).to_bytes(4, 'big') + pack('>f', tfidf)
                              for doc_id, tfidf in pl])
                # write to file(s)
                locs = writer.write(b)
    ...
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name):
        ...
        # blob_posting_locs = bucket.blob(f"body_postings_gcp/{bucket_id}_posting_locs.pickle")
