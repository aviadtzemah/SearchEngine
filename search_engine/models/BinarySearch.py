import numpy as np
import pandas as pd
from data.common import tokenize, get_posting_gen, get_posting_tokens
...
def get_candidate_documents_and_scores_binary(query_to_search, index, words, pls):
    ...
    for term in np.unique(query_to_search):
        if term in words:
            ...
            # we only need to count how many times each doc occurred
...
def binary_search_anchor(query, binary_index):
    tokenized = tokenize(query)
    words, pls = get_posting_tokens(binary_index, tokenized)
    candidates = get_candidate_documents_and_scores_binary(tokenized, binary_index, words, pls)
