...
def weighted_search_df(query, body_weight=0.2, title_weight=0.2, anchor_weight=0.2, pagerank_weight=0.2,
  ...
  """
    ...
    body_results['score'] = ((body_results['score'] - body_results['score'].min()) / (
            body_results['score'].max() - body_results['score'].min())) * body_weight

    # scaler = MinMaxScaler()  # assuming that using the same scaler won't make any problems

    title_results = search_title_df(query)
    title_results['score'] = ((title_results['score'] - title_results['score'].min()) / (
            title_results['score'].max() - title_results['score'].min())) * title_weight
    # title_results['score'] = title_weight * title_results['score']
    ...
    anchor_results['score'] = ((anchor_results['score'] - anchor_results['score'].min()) / (
            anchor_results['score'].max() - anchor_results['score'].min())) * anchor_weight
    # anchor_results['score'] = anchor_weight * anchor_results['score']
    ...
    if search_result.empty:
        return pd.DataFrame(columns=['title']).to_records(index=True)
    ...
    pageranks['score'] = ((pageranks['score'] - pageranks['score'].min()) / (
            pageranks['score'].max() - pageranks['score'].min())) * pagerank_weight
    ...
    pageviews['score'] = ((pageviews['score'] - pageviews['score'].min()) / (
            pageviews['score'].max() - pageviews['score'].min())) * pageviews_weight
...
def search_wiki(query):
  ...
  '''
    return weighted_search_df(query, 0.1, 0.4, 0.3, 0.2, 0)  # best weights
